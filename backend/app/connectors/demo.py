...
The events it produces are synthetic DATA (#9) and are tagged with the ``demo``
source id, so the write-guard + the throwaway demo store keep them fully isolated
from real cases.
"""
...
class DemoPullConnector(PullConnector):
    ...
    (it calls :meth:`generate_storyline`), not by ``poll``."""
    ...
    def manifest(cls) -> ConnectorManifest:
        ...
        return ConnectorManifest(
            ...
            display_name=gen.DEMO_SOURCE_NAME,
            category="siem",
            description=(
                "Synthetic, deterministic demo telemetry (no external system). Showcases "
                "the product with believable benign baseline + MITRE ATT&CK storylines; "
    ...
    async def poll(self, prefs: Preferences, cursor: Cursor, from_millis: int) -> list[RawEvent]:
        ...
        hits = gen.generate_window_hits(
            ...
            benign_per_hour=self._benign_per_hour,
        )
        return gen.hits_to_raw(hits, prefs)
    ...
    async def search(self, prefs: Preferences, query: StructuredQuery) -> SearchResult:
        ...
        events = gen.hits_to_raw(hits[-size:], prefs)
    ...
    def benign_batch_raw(self, rng: random.Random, ts_millis: int, count: int,
        ...
        hits = gen.generate_benign_batch(rng, self._org, ts_millis, count)
        return gen.hits_to_raw(hits, prefs)
    ...
    def storyline_raw(self, story: gen.Storyline, rng: random.Random, start_millis: int,
        ...
        events = gen.hits_to_raw(hits, prefs)
