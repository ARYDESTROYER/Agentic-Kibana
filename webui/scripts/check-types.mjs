 ...
 * schema, then compares the regenerated bytes with the files that existed at
 * gate start. A difference means the backend contract changed but the generated
 * types were not refreshed → FAIL the build with an actionable message. This is the
 * mechanical enforcement of AGENTS.md's "keep types.ts in sync with models.py"
...
// Snapshot the checked-in/working-tree artifacts before regeneration. Comparing
// against these bytes (rather than HEAD) keeps the gate useful while preparing a
// commit: a correctly regenerated but not-yet-committed contract should pass.
const before = new Map(
  TARGETS.map((target) => {
    const path = resolve(WEBUI_DIR, target);
    return [target, existsSync(path) ? readFileSync(path) : null];
  }),
);
...
const changed = TARGETS.filter((target) => {
  const current = readFileSync(resolve(WEBUI_DIR, target));
