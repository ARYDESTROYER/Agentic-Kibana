 ...
 * tiles (Open / Needs human) + the four severity-band tiles (Critical / High / Medium /
...
const CASES = [
  ...
  { case_id: 'c-med', case_number: 'T-3', title: 'MedCase', status: 'needs_human', risk_score: 30, updated_at: '2026-06-29T02:00:00Z', tags: [], comments: [] },
...
describe('Cases — summary strip', () => {
  ...
  it('renders the six triage tiles with counts derived from the loaded set', async () => {
    ...
    // Two lifecycle-status tiles: Open (2) · Needs human (1).
    expect(within(tile('cases-summary-open')).getByText('2')).toBeInTheDocument();
  ...
  it('the Open status tile narrows to open cases', async () => {
    renderCases();
    await waitFor(() => expect(screen.getByText('CritCase')).toBeInTheDocument());
    ...
    // The two open cases survive; the needs_human + closed cases are filtered out.
    expect(screen.getByText('CritCase')).toBeInTheDocument();
    expect(screen.getByText('HighCase')).toBeInTheDocument();
    expect(screen.queryByText('MedCase')).toBeNull();
    expect(screen.queryByText('LowCase')).toBeNull();
