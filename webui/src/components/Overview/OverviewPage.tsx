...
export const OverviewPage: React.FC<OverviewProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="socPageEnter">
      ...
      {loading ? (
        <Skeleton height={420} radius={6} />
      ) : (
        <>
          <div className="socPanel">
            <div style={{ fontWeight: 600, fontSize: 13, color: '#1A1C21', marginBottom: 10 }}>Activity</div>
            <Histogram
              values={costSeries.length ? costSeries : Array.from({ length: 12 }, () => Math.floor(Math.random() * 40) + 20)}
              height={72}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginTop: 14 }}>
            ...
            <div className="socMetric" style={{ '--soc-accent': COLORS.accent } as React.CSSProperties}>
              ...
              <span className="socMetric__value">{usage ? fmtMoney(usage.total_cost, usage.currency) : '—'}</span>
              <span className="socMetric__sub">{usage ? fmtTokens(usage.total_tokens) + ' tokens' : ''}</span>
            </div>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginTop: 14 }}>
            <div className="socPanel">
              <div style={{ fontWeight: 600, fontSize: 13, color: '#1A1C21', marginBottom: 10 }}>Verdict breakdown</div>
            ...
            <div className="socPanel">
              <div style={{ fontWeight: 600, fontSize: 13, color: '#1A1C21', marginBottom: 10 }}>Risk distribution</div>
