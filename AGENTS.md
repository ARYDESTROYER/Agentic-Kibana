...
npm run docs:check                         # validate app 0.1.5 ↔ bundled docs 0.1
...
- **UI design system (the suite's shared look — use it, don't re-roll it):**
  ...
  - **Cross-cutting design-system exports** live under
    ...
    future agent/MCP tooling—version 0.1.5 does **not** ship an MCP server.
...
entry. The product is now Version **`0.1.5`**: changes integrate and pass acceptance on
`Testing`, then the exact accepted commit promotes to the Stable `main` branch and receives
the immutable `v0.1.5` tag. Use `git log -1`, `VERSION`, and the latest `Journal.md` entry for the
...
`main` as its default, and has `v0.1.1` as the last installable Stable tag. The
immutable `v0.1.4` tag is a failed publication record with documentation but no
GitHub Release, signed plan, signature, digest, or GHCR release image. `v0.1.5` is
created only from the fully verified promoted 0.1.5 commit. Repository-level branch
