...
- **Status:** Round 10 and the additive Security Command Center / Case Manager work are
  ...
  The current source version is **`0.1.6`**. It is a Stable patch candidate on
  `Testing` and a Stable release only when the exact accepted `main` commit has the
  immutable `v0.1.6` tag and matching artifacts. `Testing` is the permanent integration and
  ...
  `Testing` for integration. The immutable `v0.1.6` tag is created only from the
  ...
  installable and must never be moved or reused. See
...
- **Release identity and supervised updates:** the top-right badge always reads
  `v0.1.6 · Testing|Stable`; its popover reconciles build-time Console and public backend
  ...
  requires one manual updater bootstrap to install the strictly newer signed 0.1.6
  release after its canonical Release, signature, and public images verify. A Testing/source-built 0.1.3 deployment
  or either non-installable 0.1.4/0.1.5 publication attempt must instead be reconciled manually;
  it cannot be relabelled Stable. An already-running 0.1.6 deployment cannot bootstrap
