...
- **Status:** Round 10 and the additive Security Command Center / Case Manager work are
  ...
  The current source version is **`0.1.8`**. It is a Stable patch candidate on
  `Testing` and a Stable release only when the exact accepted `main` commit has the
  immutable `v0.1.8` tag and matching artifacts. `Testing` is the permanent integration and
  ...
  `Testing` for integration. The immutable `v0.1.8` tag is created only from the
  ...
  socket ownership change. Both published records remain immutable evidence, are
  superseded, and are not supported bootstrap sources. See
...
- **Release identity and supervised updates:** the top-right badge always reads
  `v0.1.8 · Testing|Stable`; its popover reconciles build-time Console and public backend
  ...
  requires one manual updater bootstrap to install the strictly newer signed 0.1.8
  ...
  bootstrap-blocked 0.1.6/0.1.7 artifact records must instead be reconciled manually;
  it cannot be relabelled Stable. An already-running 0.1.8 deployment cannot bootstrap
