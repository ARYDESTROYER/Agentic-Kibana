...
1. After every signed-publication and canonical runtime acceptance gate passes, check
   out the accepted `v0.1.11` tag for Stable; use the `Testing` branch only for
   acceptance testing.
...
immutable `v0.1.11` tag, matching signed/public artifacts, and completed canonical
...
Use `v0.1.11` only when its complete
...
Archive is a desired target, not an active pipeline in 0.1.11. Build a separate
...
Backend and web images use the machine version `0.1.11` and accept OCI version,
...
`TLSOC_RELEASE_CHANNEL=testing`; the accepted `main`/`v0.1.11` build must explicitly
set it to `stable`. This preserves the same `0.1.11` candidate identity through
...
This support is deliberately narrow in 0.1.11: the reference single-replica standalone
...
The v0.1.1→v0.1.11 bootstrap must run from the clean, exact annotated `v0.1.11` tag
whose commit remains contained in `origin/main`. It installs the initial supervisor
transport and then delegates the complete v0.1.11 transition to the signed release plan
...
build platform while the final nginx stage remains target-platform-specific. None of
these corrections changes updater protocol 1, the keyless workflow identity, process
...
publication attempts, the published-but-bootstrap-blocked `v0.1.6` through `v0.1.9`
records, and the failed-publication `v0.1.10` record are
...
unchanged v0.1.1 application, the v0.1.11 bootstrap replaces that exact-version
mismatch before signed-plan verification. Reconcile other states through the
documented 0.1.11 path appropriate to their installed release. Because bootstrap
requires a strictly newer target, an already-running 0.1.11 cannot bootstrap from the
0.1.11 plan.
