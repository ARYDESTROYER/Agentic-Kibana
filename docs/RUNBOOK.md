...
#   -> {"status":"ok","version":"0.1.10","es_connected":...,"store_type":"...","setup_complete":true}
...
- Use **Settings → Organization → Storage & retention** to preview and explicitly
  apply the owned-state ILM policy. In 0.1.10 it applies only to the append-only audit
