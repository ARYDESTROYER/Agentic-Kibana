...
!!! info "Matched to this app"
    ...
    **Documentation 0.1** describes **Agentic SOC 0.1.5**. The application bundles this
...
Stable**. The current source version uses SemVer `0.1.5`; compatible documentation
uses the major.minor line `0.1`.

The repository uses default `main` for accepted source and retains `Testing` for
integration. Version 0.1.5 is Stable only when the exact accepted `main` commit has
the immutable `v0.1.5` tag and matching signed/public artifacts. Repository
...
branch, workflow run, or public URL as accepted. The immutable `v0.1.4` tag is a
failed, non-installable publication record: it has documentation but no GitHub
Release, signed plan, or release image. Read
[Agentic SOC 0.1.5](releases/0.1.5.md),
