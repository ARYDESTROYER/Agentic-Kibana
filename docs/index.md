...
!!! info "Matched to this app"
    ...
    **Documentation 0.1** describes **Agentic SOC 0.1.11**. The application bundles this
...
Stable**. The current source version uses SemVer `0.1.11`; compatible documentation
uses the major.minor line `0.1`.

The repository uses default `main` for accepted source and retains `Testing` for
integration. Version 0.1.11 is Stable only when the exact accepted `main` commit has
the immutable `v0.1.11` tag and matching signed/public artifacts and every canonical
...
artifact set or GitHub Release existed. Version 0.1.11 remains a candidate until
its full signed-publication and canonical acceptance evidence exists. Read
[Agentic SOC 0.1.11](releases/0.1.11.md),
