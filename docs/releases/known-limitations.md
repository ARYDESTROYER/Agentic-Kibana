...
description: Release gates and explicit operating constraints for Agentic SOC 0.1.8.
---

# Known limitations

This list is part of the product contract for Agentic SOC `0.1.8` in Testing and,
...
published `v0.1.8` images and signed assets work through a real PostgreSQL Compose
...
0.1.8 carries the version-1 supervisor protocol and canonical base-Compose contract
...
`scripts/bootstrap-updater.sh` step from the clean, exact annotated `v0.1.8` tag whose
commit remains contained in `origin/main`; that step delegates the full
v0.1.1→v0.1.8 transition to the signed plan. Testing/source-built 0.1.3 and the
non-installable v0.1.4/v0.1.5 attempts, plus the published-but-bootstrap-blocked
v0.1.6 and v0.1.7 records, must instead be reconciled manually and cannot be
...
shipped capability boundary; 0.1.8 corrects that startup contract and adds a
fail-closed Linux runtime gate.
...
The updater foundation included in version 0.1.8 retains updater jobs, preflights,
