...
- 2026-06-30 — R2-W1 done & verified: all 5 bugs fixed (gauge→dasharray ring, 21 tests; MFA QR format-info fix + clipboard fallback, 7 tests; removed duplicate close X; chat full-height frame; store-degraded tooltip). webui build clean, 50 vitest, 0 rules-of-hooks errors. Committed 9ab2954.
- 2026-06-30 — R2-W2 done & verified (committed 317bd5a): login redesign (split brand hero + aurora, password-strength, segmented OTP, SSO brand icons; all 4 submit handlers byte-identical) + account self-service (User additive profile fields, /api/account/me + /api/me/avatar, avatar Canvas-resize + server validate, env-managed read-only, secrets-never-leak, user menu in shell). Backend 649→666 green; webui build + 58 vitest + 0 lint errors. Next: R2-W3 (sessions + token policy).
  - W3 BACK-COMPAT GUARDRAIL: require_auth's new async session check MUST no-op when auth off, and DENY ONLY on explicit revocation / tv mismatch / policy expiry; an unknown sid on a validly-signed token is lazily registered (not rejected) so direct-token tests + the 666 baseline stay green. verify() stays sync/IO-free.
- 2026-06-30 — R2-W3 sessions+policy committed 88cb3c6 (682 green; auth-off no-op + unknown-sid lazy-register verified). R2-W4 Settings IA + page consolidation committed 9eb7d57 (Account/Users/Security/SSO/Sessions under Settings; nav <=5 groups; backend untouched). R2-W5 Demo Mode committed 93ac735 (697 green; isolated separate InMemory store + $0 mock LLM + real-cursor-gated + reversible; decide() byte-identical). R2-W6 source multi-feed committed 2ada050 (716 green; events/alerts/ignore + per-feed query/mapping/severity-floor/cursors; legacy back-compat + #4 no-drop proven). R2-W7a email Resend+SES+templates committed f0909af (744 green; escaping + send-never-raises proven). R2-W7b customization committed 36ff656 (758 green; UserPrefsStore + saved views + columns + terminology + theme; org<-user cascade + admin gates). R2-W7c best-of-best UX committed 5869f13 (772 green; Cmd-K palette + global search + bulk actions(#3-safe) + audit viewer).
- 2026-06-30 — All R2 feature waves DONE. Next: R2-Final (adversarial audit fleet + docs fleet + full green sweep + demo walkthrough). Baseline: backend 772 pytest, webui build + 86 vitest, 0 rules-of-hooks lint errors, case_manager byte-identical, zero new deps.
- 2026-06-30 — R2-Final done: 16-agent adversarial audit + 6-agent docs fleet (committed aae7a76). Audit found real bugs → 8 confirmed fixes auto-applied (mostly missing RBAC gates, no-ops in default profile) + docs refreshed + ROUND2_AUDIT.md written. Then R2-Remediation (committed 763ded9) fixed the confirmed HIGH/MEDIUM review items: #4 feed cursor starvation, demo-chat isolation, env-admin token-version lockout, set_status->RESOLVED RBAC, email template text_safe/{{{}}}/branding-svg hardening, and a strengthened authZ-coverage CI test (+22 regression tests). FINAL: backend **794 pytest green**, webui build + 86 vitest, 0 lint errors, case_manager byte-identical, zero new deps. ROUND 2 COMPLETE.
  - Deferred (documented in ROUND2_AUDIT.md, low/speculative): session-KV optimistic-concurrency for revoke races; multi-generation refresh-reuse detection; CONFIG_INDEX nested-type-collision (ES-only); deep-link breadcrumb cosmetic. → next-round candidates with the best-of-best Tier-2/3 (API keys, dashboards builder, scheduled reports, watchlists, SLA timers, hunting/query builder).
