...
before application version `v0.1.4` is tagged.
...
source. Version 0.1.4 is a candidate on `Testing` and is Stable only when the
resulting verified `main` commit has the immutable `v0.1.4` tag and matching
...
Verify the one-time v0.1.1→v0.1.4 bootstrap separately from later Console updates:
...
the initial supervisor transport before delegating the full v0.1.4 transition to the
...
For the 0.1 line it checks that the root `VERSION` (`0.1.4`) agrees with backend,
...
5. Build and stamp the verified commit with the correct channel, exact SHA, build
   date, and source URL.
6. Create the immutable `v0.1.4` tag and publish matching application artifacts by
